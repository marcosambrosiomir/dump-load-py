...
function _tabanalysCountCellLabel(row,column){const initialDefined=row&&row.initial_count!==null&&row.initial_count!==undefined;const finalDefined=row&&row.final_count!==null&&row.final_count!==undefined;const initialValue=initialDefined?row.initial_count:null;const finalValue=finalDefined?row.final_count:null;const currentValue=column==='initial'?initialValue:finalValue;let label=_tabanalysCountLabel(currentValue);if(initialValue===null&&finalValue===null){return label;}if(initialValue===finalValue){return label;}if(column==='initial'&&initialValue!==null&&(finalValue===null||initialValue>finalValue)){return label+'(+)';}if(column==='final'&&finalValue!==null&&(initialValue===null||finalValue>initialValue)){return label+'(+)';}return label;}
function _tabanalysDiffValue(row){const initial=row&&typeof row.initial_count==='number'?row.initial_count:0;const final=row&&typeof row.final_count==='number'?row.final_count:0;if((row.initial_count===null||row.initial_count===undefined)&&(row.final_count===null||row.final_count===undefined)){return null;}return initial-final;}
function _tabanalysDiffLabel(row){const diff=_tabanalysDiffValue(row);return diff===null?'--':String(diff);}
function _tabanalysHasDifference(row){const diff=_tabanalysDiffValue(row);return diff!==null&&diff!==0;}
function _tabanalysTableHasIssue(row){return !!(row&&row.table_status&&row.table_status!=='ok');}
function _tabanalysTableNeedsAttention(row){return _tabanalysHasDifference(row)||_tabanalysTableHasIssue(row);}
function _tabanalysRowMarkup(row){const diff=_tabanalysDiffValue(row);const hasIssue=_tabanalysTableHasIssue(row);const rowClass=_tabanalysTableNeedsAttention(row)?'has-difference':'';const diffClass=diff===null?'':(diff===0?'is-zero':(diff>0?'is-positive':'is-negative'));const critiqueClass=hasIssue?'is-negative':'is-positive';return '<tr class="'+rowClass+'"><td>'+escapeHtml(row.name||'')+'</td><td>'+escapeHtml(row.area||'')+'</td><td class="tabanalys-number">'+escapeHtml(_tabanalysCountCellLabel(row,'initial'))+'</td><td class="tabanalys-number">'+escapeHtml(_tabanalysCountCellLabel(row,'final'))+'</td><td class="tabanalys-number '+diffClass+'">'+escapeHtml(_tabanalysDiffLabel(row))+'</td><td class="'+critiqueClass+'">'+escapeHtml(row.table_critique||'OK')+'</td></tr>';}
function _tabanalysIndexHasIssue(row){return (row&&row.status)==='warning';}
function _tabanalysIndexStatusLabel(row){return _tabanalysIndexHasIssue(row)?'Fora da area esperada':'OK';}
function _tabanalysIndexRowMarkup(row){const rowClass=_tabanalysIndexHasIssue(row)?'has-difference':'';const statusClass=_tabanalysIndexHasIssue(row)?'is-negative':'is-positive';return '<tr class="'+rowClass+'"><td>'+escapeHtml(row.table_name||'')+'</td><td>'+escapeHtml(row.table_area||'')+'</td><td>'+escapeHtml(row.index_name||'')+'</td><td>'+escapeHtml(row.index_area||'')+'</td><td>'+escapeHtml(row.expected_area||'')+'</td><td class="'+statusClass+'">'+escapeHtml(_tabanalysIndexStatusLabel(row))+'</td></tr>';}
function _tabanalysTablesMarkup(database,onlyDifferences,selectedArea){const rows=Array.isArray(database&&database.rows)?database.rows:[];const areaFilteredRows=selectedArea?rows.filter(function(row){return (row.area||'')===selectedArea||(row.initial_area||'')===selectedArea||(row.final_area||'')===selectedArea;}):rows;const filteredRows=onlyDifferences?areaFilteredRows.filter(_tabanalysTableNeedsAttention):areaFilteredRows;const emptyMessage=selectedArea?'Nenhum registro encontrado para a area selecionada.':'Nenhuma diferenca ou critica encontrada para este banco.';const content=filteredRows.length?'<div class="tabanalys-table-wrap"><table class="tabanalys-table"><thead><tr><th>Tabela</th><th>Area</th><th>Inicial</th><th>Final</th><th>Diferenca</th><th>Areas</th></tr></thead><tbody>'+filteredRows.map(_tabanalysRowMarkup).join('')+'</tbody></table></div>':'<div class="start-modal-loading">'+emptyMessage+'</div>';return {content:content,count:filteredRows.length,total:rows.length,meta:'Inicial: '+escapeHtml(database.has_initial?'ok':'nao encontrado')+' • Final: '+escapeHtml(database.has_final?'ok':'nao encontrado')};}
function _tabanalysIndexesMarkup(database,onlyIssues,selectedArea){const rows=Array.isArray(database&&database.index_rows)?database.index_rows:[];const areaFilteredRows=selectedArea?rows.filter(function(row){return (row.index_area||'')===selectedArea||(row.expected_area||'')===selectedArea||(row.table_area||'')===selectedArea;}):rows;const filteredRows=onlyIssues?areaFilteredRows.filter(_tabanalysIndexHasIssue):areaFilteredRows;const emptyMessage=selectedArea?'Nenhum indice encontrado para a area selecionada.':'Nenhuma critica de indice encontrada para este banco.';const content=filteredRows.length?'<div class="tabanalys-table-wrap"><table class="tabanalys-table"><thead><tr><th>Tabela</th><th>Area tabela</th><th>Indice</th><th>Area indice</th><th>Area esperada</th><th>Status</th></tr></thead><tbody>'+filteredRows.map(_tabanalysIndexRowMarkup).join('')+'</tbody></table></div>':'<div class="start-modal-loading">'+emptyMessage+'</div>';return {content:content,count:filteredRows.length,total:rows.length,meta:'DF: '+escapeHtml(database.has_df?'ok':'nao encontrado')};}
function _tabanalysDatabaseMarkup(database,currentView,onlyFiltered,selectedArea){const section=currentView==='indexes'?_tabanalysIndexesMarkup(database,onlyFiltered,selectedArea):_tabanalysTablesMarkup(database,onlyFiltered,selectedArea);return '<section class="tabanalys-db-card"><div class="tabanalys-db-head"><div><strong>'+escapeHtml(database.db_name||'')+'</strong><div class="tabanalys-db-meta">'+section.meta+'</div></div><div class="tabanalys-db-meta">'+escapeHtml(String(section.count))+' de '+escapeHtml(String(section.total))+' registro(s)</div></div>'+section.content+'</section>';}
function _tabanalysSelectorMarkup(databases,selectedDbName){if(databases.length<=1){return '';}return '<label class="tabanalys-select-wrap"><span>Banco</span><select id="tabanalys-db-select" class="tabanalys-select">'+databases.map(function(database){const selected=database.db_name===selectedDbName?' selected':'';return '<option value="'+escapeHtml(database.db_name||'')+'"'+selected+'>'+escapeHtml(database.db_name||'')+'</option>';}).join('')+'</select></label>';}
function _tabanalysViewTabsMarkup(currentView){return '<div class="tabanalys-view-tabs"><button type="button" class="tabanalys-view-tab'+(currentView==='tables'?' active':'')+'" data-tabanalys-view="tables">Tabelas</button><button type="button" class="tabanalys-view-tab'+(currentView==='indexes'?' active':'')+'" data-tabanalys-view="indexes">Indices</button></div>';}
function _tabanalysAreaSelectorMarkup(database,selectedArea,currentView){const rows=currentView==='indexes'?Array.isArray(database&&database.index_rows)?database.index_rows:[]:Array.isArray(database&&database.rows)?database.rows:[];const areas=(currentView==='indexes'?rows.reduce(function(list,row){['table_area','index_area','expected_area'].forEach(function(key){if(row&&row[key]){list.push(row[key]);}});return list;},[]):rows.map(function(row){return row.area||'';})).filter(Boolean).filter(function(area,index,list){return list.indexOf(area)===index;}).sort();if(!areas.length){return '';}return '<label class="tabanalys-select-wrap"><span>Area</span><select id="tabanalys-area-select" class="tabanalys-select"><option value="">Todas</option>'+areas.map(function(area){const selected=area===selectedArea?' selected':'';return '<option value="'+escapeHtml(area)+'"'+selected+'>'+escapeHtml(area)+'</option>';}).join('')+'</select></label>';}
function _renderTabanalysPayload(payload,stateOverride){const meta=document.getElementById('tabanalys-meta');const content=document.getElementById('tabanalys-content');const filter=document.getElementById('tabanalys-only-differences');const selectedDb=document.getElementById('tabanalys-db-select');const selectedAreaControl=document.getElementById('tabanalys-area-select');const selectedViewControl=document.querySelector('[data-tabanalys-view].active');const databases=Array.isArray(payload&&payload.databases)?payload.databases:[];const currentState=stateOverride||{};const currentView=currentState.view||((selectedViewControl&&selectedViewControl.getAttribute('data-tabanalys-view'))||'tables');const onlyFiltered=typeof currentState.onlyFiltered==='boolean'?currentState.onlyFiltered:!!(filter&&filter.checked);const selectedDbName=currentState.selectedDbName||((selectedDb&&selectedDb.value)||((databases[0]&&databases[0].db_name)||''));const visibleDatabases=selectedDbName?databases.filter(function(database){return database.db_name===selectedDbName;}):databases;const currentDatabase=visibleDatabases[0]||null;const selectedArea=Object.prototype.hasOwnProperty.call(currentState,'selectedArea')?currentState.selectedArea:((selectedAreaControl&&selectedAreaControl.value)||'');const filterLabel=currentView==='indexes'?'Mostrar somente criticas':'Mostrar somente diferencas/criticas';meta.textContent='Job '+escapeHtml(payload.job_id||'')+' • '+escapeHtml(String(databases.length))+' banco(s)';if(!databases.length){content.innerHTML='<div class="start-modal-loading">Nenhum banco com Tabanalys encontrado para este job.</div>';return;}content.innerHTML='<div class="tabanalys-toolbar">'+_tabanalysViewTabsMarkup(currentView)+_tabanalysSelectorMarkup(databases,selectedDbName)+_tabanalysAreaSelectorMarkup(currentDatabase,selectedArea,currentView)+'<label class="tabanalys-filter"><input type="checkbox" id="tabanalys-only-differences"'+(onlyFiltered?' checked':'')+'>'+filterLabel+'</label></div>'+visibleDatabases.map(function(database){return _tabanalysDatabaseMarkup(database,currentView,onlyFiltered,selectedArea);}).join('');const nextFilter=document.getElementById('tabanalys-only-differences');if(nextFilter){nextFilter.addEventListener('change',function(){_renderTabanalysPayload(payload,{view:currentView,selectedDbName:selectedDbName,selectedArea:selectedArea,onlyFiltered:nextFilter.checked});});}const nextDbSelect=document.getElementById('tabanalys-db-select');if(nextDbSelect){nextDbSelect.addEventListener('change',function(){_renderTabanalysPayload(payload,{view:currentView,selectedDbName:nextDbSelect.value,selectedArea:'',onlyFiltered:onlyFiltered});});}const nextAreaSelect=document.getElementById('tabanalys-area-select');if(nextAreaSelect){nextAreaSelect.addEventListener('change',function(){_renderTabanalysPayload(payload,{view:currentView,selectedDbName:selectedDbName,selectedArea:nextAreaSelect.value,onlyFiltered:onlyFiltered});});}document.querySelectorAll('[data-tabanalys-view]').forEach(function(button){button.addEventListener('click',function(){_renderTabanalysPayload(payload,{view:button.getAttribute('data-tabanalys-view'),selectedDbName:selectedDbName,selectedArea:'',onlyFiltered:onlyFiltered});});});}
function openTabanalysModal(){const modal=document.getElementById('tabanalys-modal');const meta=document.getElementById('tabanalys-meta');const content=document.getElementById('tabanalys-content');if(!modal||!meta||!content){return;}modal.classList.add('open');meta.textContent='Carregando comparativo do Tabanalys...';content.innerHTML='<div class="start-modal-loading">Lendo Tabanalys inicial e final do job atual...</div>';const query=activeJob&&activeJob.job_id?('?job_id='+encodeURIComponent(activeJob.job_id)):'';fetch('/job/tabanalys'+query,{cache:'no-store'}).then(function(r){if(r.status===404){throw new Error('Nenhum job com Tabanalys disponivel.');}return r.json();}).then(function(payload){_renderTabanalysPayload(payload);}).catch(function(err){content.innerHTML='<div class="error-box">'+escapeHtml(String(err))+'</div>';});}
"""
...
def render_html(report):
    ...
    return (
        ...
        ".tabanalys-toolbar{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin:0 0 12px}"
        ".tabanalys-view-tabs{display:inline-flex;align-items:center;gap:6px;padding:4px;border:1px solid #d5dbe3;border-radius:12px;background:#fff}"
        ".tabanalys-view-tab{border:0;background:transparent;color:#475569;padding:8px 12px;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer}"
        ".tabanalys-view-tab.active{background:#0f172a;color:#fff}"
        ".tabanalys-select-wrap{display:inline-flex;align-items:center;gap:8px;color:#334155;font-size:13px;font-weight:700}"
        ".tabanalys-select{min-width:180px;padding:8px 12px;border:1px solid #d5dbe3;border-radius:10px;background:#fff;color:#334155;font-size:13px;font-weight:600}"
        ".tabanalys-filter{display:inline-flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid #d5dbe3;border-radius:10px;background:#fff;color:#334155;font-size:13px;font-weight:700}"
        ".tabanalys-filter input{width:16px;height:16px;margin:0}"
        ".tabanalys-db-card{padding:14px;border:1px solid #d8dee7;border-radius:14px;background:#f8fafc;margin-bottom:12px}"
        ...
        ".tabanalys-db-head strong{display:block;color:#0f172a;font-size:15px}"
        ".tabanalys-db-meta{color:#64748b;font-size:12px;line-height:1.5}"
        ".tabanalys-table-wrap{overflow:auto;border:1px solid #d8dee7;border-radius:12px;background:#fff}"
        ".tabanalys-table{width:100%;border-collapse:separate;border-spacing:0;min-width:720px;background:#fff}"
        ".tabanalys-table th,.tabanalys-table td{padding:9px 12px;border-bottom:1px solid #e7ecf2;text-align:left;font-size:13px;color:#334155}"
        ".tabanalys-table th{background:#eef3f8;color:#334155;position:sticky;top:0;font-weight:800}"
        ".tabanalys-table td{background:#fff}"
        ".tabanalys-table tr:nth-child(even) td{background:#f9fbfd}"
        ".tabanalys-table tr.has-difference td{background:#fffdf3}"
        ".tabanalys-number{text-align:right;font-variant-numeric:tabular-nums}"
        ".tabanalys-table td.is-zero{color:#64748b}"
        ".tabanalys-table td.is-positive{color:#0f766e;font-weight:700}"
        ".tabanalys-table td.is-negative{color:#b91c1c;font-weight:700}"
