...
from app.jobs import get_current_job_summary, get_job_log, get_job_log_chunk, get_job_summary, start_dry_run_job, start_dump_job
...
def render_html(report):
    ...
    return (
        ...
        ".job-head{display:flex;justify-content:space-between;align-items:flex-start;gap:16px;padding:18px 20px 0}"
        ".job-title{font-size:18px;font-weight:800;letter-spacing:-.02em;color:#f8fafc}"
        ".job-subtitle{margin-top:4px;color:#cbd5e1;font-size:14px}"
        ".job-badge{display:inline-flex;align-items:center;padding:8px 12px;border-radius:999px;font-size:12px;font-weight:800;letter-spacing:.02em}"
        ".job-badge.idle{background:#334155;color:#e2e8f0}"
        ".job-badge.running{background:#1d4ed8;color:#eff6ff}"
        ".job-badge.completed{background:#065f46;color:#d1fae5}"
        ".job-badge.warning{background:#92400e;color:#ffedd5}"
        ".job-badge.failed{background:#7f1d1d;color:#fee2e2}"
        ...
        "<div class='job-head'><div><div class='job-title'>Andamento</div></div><div id='job-badge' class='job-badge idle'>Nenhuma atividade em andamento</div></div>"
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if parsed_url.path in ("/catalogo-comandos", "/comandos"):
                ...
                html = build_catalog_page(catalog).encode("utf-8")
                self.send_response(200)
        ...
        def do_POST(self):
            ...
            if parsed_url.path in ("/catalogo-comandos/save", "/comandos/save"):
                ...
                html = build_catalog_page(load_catalog(CATALOG_PATH), "Catálogo salvo com sucesso.").encode("utf-8")
                self.send_response(200)
                self.send_header("Content-Type", "text/html; charset=utf-8")
                self.send_header("Content-Length", str(len(html)))
                self.end_headers()
            ...
            if parsed_url.path in ("/catalogo-comandos-load/save", "/comandos-load/save"):
                ...
                html = build_catalog_page(
                    load_catalog(LOAD_CATALOG_PATH),
