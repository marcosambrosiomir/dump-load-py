...
podman run -d --ipc=host --pid=host --uidmap 0:0:4294967295 --gidmap 0:0:4294967295 --name "$CONTAINER_NAME" -p "${APP_HOST_IP}:${APP_HOST_PORT}:8000" -v "${CONFIG_DIR}:/app/config:rw" -v "${RUNTIME_DIR}:/app/runtime:rw" -v "${LOGS_DIR}:/app/logs:rw" -v "/:${HOST_ROOT_MOUNT}:rw" -v /totvs/database:/totvs/database:rw ${DLC_MOUNT} ${DB_ROOT_MOUNT} ${TEMP_DIR_MOUNT} "$IMAGE_NAME"
